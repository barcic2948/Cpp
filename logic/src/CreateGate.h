...
public:
    ...
    std::vector<Gate> r_edge(std::string name, std::string clock_input);

    std::vector<Gate> sr_flipFlop(std::string name, std::string set_input, std::string reset_input);

    std::vector<Gate> d_latch(std::string name, std::string D_input, std::string clock_input);
...
std::vector<Gate> CreateGate::r_edge(std::string name, std::string clock_input) {
    std::vector<Gate> rez;
    rez.push_back(Gate(name+".out", "xor", {clock_input, name+".Q"}, false, "trace"));
...
std::vector<Gate> CreateGate::sr_flipFlop(std::string name, std::string set_input, std::string reset_input) {
    std::vector<Gate> rez;
    rez.push_back(Gate(name+".Q", "nor", {reset_input, name+".~Q"}, false));
    rez.push_back(Gate(name+".~Q", "nor", {set_input, name+".Q"}, true));
    return rez;
}

std::vector<Gate> CreateGate::d_latch(std::string name, std::string D_input, std::string write_input) {
    std::vector<Gate> rez;
    rez.push_back(Gate(name+".~D", "not", {D_input}, false));
    rez.push_back(Gate(name+".DC_AND", "and", {D_input, write_input}, false));
    rez.push_back(Gate(name+".~DC_AND", "and", {name+".~D", write_input}, false));
...
std::vector<Gate> CreateGate::d_flipFlop(std::string name, std::string D_input, std::string clock_input) {
    std::vector<Gate> rez;
    rez = connect(rez, r_edge(name+".timer", clock_input));
    rez = connect(rez, d_latch(name, D_input, name+".timer.out"));
    return rez;
}
...
std::vector<Gate> CreateGate::counter_4b(std::string name, std::string clock_input) {
    ...
    rez = connect(rez, d_flipFlop(name+"0", name+"0.~Q", clock_input));
    rez = connect(rez, d_flipFlop(name+"1", name+"0.xor", clock_input));
    rez = connect(rez, d_flipFlop(name+"2", name+"1.xor", clock_input));
    rez = connect(rez, d_flipFlop(name+"3", name+"2.xor", clock_input));
    return rez;
}
...
std::vector<Gate> CreateGate::counter_ib(int count, std::string name, std::string clock_input) {
    ...
    for(int i = 0; i < count-1; i++) {
        rez = connect(rez, d_flipFlop(name+std::to_string(i+1), name+std::to_string(i)+".xor", clock_input));
    }
    return rez;
}